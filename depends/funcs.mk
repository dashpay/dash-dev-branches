...
vendor-$(1)-crates: $(native_rust_cached) $(SOURCES_PATH)/$($(1)_file_name)
...
echo "$(rust_stdlib_sha256_hash_$(1))  rust-std-$(rust_stdlib_version)-$(1).tar.gz" > "$(SOURCES_PATH)/.rust-std-$(1).hash" && \
cd "$(SOURCES_PATH)" && $(build_SHA256SUM) -c ".rust-std-$(1).hash" && rm -f ".rust-std-$(1).hash" && \
...
([ -f "$(SOURCES_PATH)/$($(1)_file_name)" ] && \
  ...
  (echo "Downloading $($(1)_file_name)..." && \
    $(build_DOWNLOAD) "$(SOURCES_PATH)/$($(1)_file_name)" "$($(1)_download_path)/$(or $($(1)_download_file),$($(1)_file_name))")) && \
echo "$($(1)_sha256_hash)  $($(1)_file_name)" > "$(SOURCES_PATH)/.$(1).hash" && \
cd "$(SOURCES_PATH)" && $(build_SHA256SUM) -c ".$(1).hash" && rm -f ".$(1).hash"
endef
