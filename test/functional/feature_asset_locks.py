...
class AssetLocksTest(DashTestFramework):
    ...
    def create_assetunlock(self, mninfo, index, withdrawal, pubkey=None):
    ...
    def run_test(self):
        ...
        node.generate(1)
        assert_equal(self.get_credit_pool_amount(), locked_1)
        # Generate a number of blocks to ensure this is the longest chain for later in the test when we reconsiderblock
        node.generate(12)
        self.sync_all()

        assert_equal(self.get_credit_pool_amount(), locked_1)
        assert_equal(self.get_credit_pool_amount(node=self.nodes[1]), locked_1)
        ...
        assert_equal(self.get_credit_pool_amount(), locked_2)
        ...
        asset_unlock_tx_index_too_far = self.create_assetunlock(self.mninfo, 10001, COIN, pubkey)
        ...
        assert_equal(self.get_credit_pool_amount(), locked_1)
        asset_unlock_tx = self.create_assetunlock(self.mninfo, 101, COIN, pubkey)
        asset_unlock_tx_late = self.create_assetunlock(self.mninfo, 102, COIN, pubkey)
        asset_unlock_tx_too_late = self.create_assetunlock(self.mninfo, 103, COIN, pubkey)
        ...
        assert_equal(self.get_credit_pool_amount(), locked_1 - 2 * COIN)
        ...
        node.generate(1)
        self.sync_all()
        assert_equal(self.get_credit_pool_amount(), locked_1 - 3 * COIN)
        ...
        assert_equal(self.get_credit_pool_amount(), locked_1 - 3 * COIN)
        assert_equal(self.get_credit_pool_amount(block_hash=block_hash_1), locked_1)

        # too big withdrawal should not be mined
        asset_unlock_tx_full = self.create_assetunlock(self.mninfo, 201, 1 + self.get_credit_pool_amount(), pubkey)
        ...
        self.mempool_size += 1
        asset_unlock_tx_full = self.create_assetunlock(self.mninfo, 301, self.get_credit_pool_amount(), pubkey)
        self.check_mempool_result(tx=asset_unlock_tx_full, result_expected={'allowed': True })
        ...
        for next_amount in withdrawals:
            index += 1
            asset_unlock_tx = self.create_assetunlock(self.mninfo, index, next_amount, pubkey)
            self.send_tx(asset_unlock_tx)
            if index == 401:
                node.generate(1)
        ...
        index += 1
        asset_unlock_tx = self.create_assetunlock(self.mninfo, index, limit_amount_2, pubkey)
        self.send_tx(asset_unlock_tx)
        node.generate(1)
        self.sync_all()
        assert_equal(new_total, self.get_credit_pool_amount())
        ...
        index += 1
        asset_unlock_tx = self.create_assetunlock(self.mninfo, index, COIN * 100, pubkey)
        self.send_tx(asset_unlock_tx)
        node.generate(1)
        self.sync_all()
