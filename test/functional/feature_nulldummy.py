...
class NULLDUMMYTest(BitcoinTestFramework):
    ...
    def block_submit(self, node, txs, accept = False):
        ...
        v19_activated = get_bip9_status(self.nodes[0], 'v19')['status'] == 'active'
        block = create_block(self.tip, create_coinbase(self.lastblockheight + 1, dip4_activated=dip4_activated, v19_activated=v19_activated), self.lastblocktime + 1)
        block.nVersion = 4
        for tx in txs:
            tx.rehash()
            block.vtx.append(tx)
