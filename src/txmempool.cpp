...
bool CTxMemPool::existsProviderTxConflict(const CTransaction &tx) const {
    ...
    if (!GetTxPayload(tx, proTx))
        assert(false);
    return mapProTxAddresses.count(proTx.addr) || mapProTxRegisterPubKeyIDs.count(proTx.keyIDOwner) || mapProTxRegisterPubKeyIDs.count(proTx.keyIDOperator) || mapProTxRegisterPubKeyIDs.count(proTx.keyIDVoting);
}
