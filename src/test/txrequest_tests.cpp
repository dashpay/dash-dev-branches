...
std::string v2s(const std::vector<CInv>& v) {
    std::string out;
    for (auto i : v) {
        if (!out.empty()) out += ", ";
        out += i.ToString();
    }
    return strprintf("v[%d] = {%s}", v.size(), out);
}
...
struct Runner
{
    ...
    std::set<CInv> txhashset;

    /** Which (peer, inv) combinations are known to be expired. These need to be accumulated here instead of
     *  checked directly in the GetRequestable return value to avoid introducing a dependency between the various
     *  parallel tests. */
    std::multiset<std::pair<NodeId, CInv>> expired;
};
...
public:
    ...
    void ReceivedInv(NodeId peer, const CInv& gtxid, bool pref, std::chrono::microseconds reqtime)
    {
        auto& runner = m_runner;
        runner.actions.emplace_back(m_now, [=,&runner]() {
    ...
    void RequestedTx(NodeId peer, const CInv& txhash, std::chrono::microseconds exptime)
    {
        auto& runner = m_runner;
        runner.actions.emplace_back(m_now, [=,&runner]() {
     ...
     *  If expected_completed is provided, verify that the call reports completing an announcement
     *  (or not) accordingly. */
    void ReceivedResponse(NodeId peer, const CInv& txhash, std::optional<bool> expected_completed = std::nullopt)
    {
        auto& runner = m_runner;
        runner.actions.emplace_back(m_now, [=,&runner]() {
            const bool completed = runner.txrequest.ReceivedResponse(peer, txhash);
            if (expected_completed.has_value()) {
                BOOST_CHECK_EQUAL(completed, *expected_completed);
            }
            runner.txrequest.SanityCheck();
        });
    }
     ...
     */
    void Check(NodeId peer, const std::vector<CInv>& expected, size_t candidates, size_t inflight,
        ...
        std::chrono::microseconds offset = std::chrono::microseconds{0})
    {
        ...
        runner.actions.emplace_back(m_now, [=,&runner]() {
            std::vector<std::pair<NodeId, CInv>> expired_now;
            ...
            if (ret != expected) {
                std::cerr << "     ret: " << v2s(ret) << std::endl;
                std::cerr << "expected: " << v2s(expected) << std::endl;
            }
            BOOST_CHECK_MESSAGE(ret == expected, "[" + comment + "] mismatching requestables");
        });
    }

    /** Verify that an announcement for the inv by peer has expired some time before this check is scheduled.
     *
     * Every expected expiration should be accounted for through exactly one call to this function.
     */
    void CheckExpired(NodeId peer, const CInv& gtxid)
    {
        const auto& testname = m_testname;
        auto& runner = m_runner;
        runner.actions.emplace_back(m_now, [=,&runner]() {
            auto it = runner.expired.find(std::pair<NodeId, CInv>{peer, gtxid});
     ...
     */
    CInv NewTxHash(const std::vector<std::vector<NodeId>>& orders = {})
    {
        CInv ret;
        bool ok;
        do {
            ret = CInv{InsecureRandBool() ? MSG_TX  : MSG_QUORUM_FINAL_COMMITMENT, InsecureRand256()};
            ok = true;
    ...
    CInv NewGTxid(const std::vector<std::vector<NodeId>>& orders = {})
    {
        return NewTxHash(orders);
    }
    ...
    if (config >> 3) { // We'll request the transaction
        ...
        if ((config >> 3) == 1) { // The request will time out
            ...
            if ((config >> 3) == 3) { // A response will arrive for the transaction
                scenario.ReceivedResponse(peer, gtxid, true);
                // A duplicate response does not complete anything anymore.
                scenario.ReceivedResponse(peer, gtxid, false);
...
void BuildPriorityTest(Scenario& scenario, int config)
{
    ...
    if (config & 16) {
        ...
        scenario.ReceivedResponse(priopeer, gtxid, true);
    }
    if (InsecureRandBool()) scenario.AdvanceTime(RandomTime8s());
...
/** Add to scenario a test that verifies behavior related to two inv types with the same hash
 *  being announced. Unlike upstream's txid/wtxid pair, distinct inv types are tracked as independent
 *  announcements, so both can be requested concurrently.
 *
...
void BuildWtxidTest(Scenario& scenario, int config)
{
    ...
    auto txhash = scenario.NewTxHash().hash;
    CInv txid{MSG_TX, txhash};
    CInv wtxid{MSG_QUORUM_FINAL_COMMITMENT, txhash};
    ...
    // Let time pass if needed, and check that both announcements are to-be-requested
    // independently (distinct inv types are separate announcements).
    ...
    if (config & 2) {
        ...
        scenario.Check(peerW, {wtxid}, 1, 0, 0, "w2");
    } else {
        scenario.Check(peerT, {txid}, 1, 0, 0, "w3");
    ...
    if (config & 2) {
        ...
        scenario.Check(peerW, {wtxid}, 1, 0, 0, "w6");
    } else {
        scenario.RequestedTx(peerW, wtxid, scenario.Now() + expiry);

        scenario.Check(peerT, {txid}, 1, 0, 0, "w7");
    ...
    if (config & 2) {
        scenario.Check(peerT, {}, 0, 0, 0, "w9");
        scenario.Check(peerW, {wtxid}, 1, 0, 0, "w10");
        scenario.CheckExpired(peerT, txid);
    } else {
        scenario.Check(peerT, {txid}, 1, 0, 0, "w11");
        scenario.Check(peerW, {}, 0, 0, 0, "w12");
        scenario.CheckExpired(peerW, wtxid);
    }

    // Once the object arrives, the caller forgets each inv type it may have been
    // announced under, and all announcements are gone.
    if (InsecureRandBool()) scenario.AdvanceTime(RandomTime8s());
    scenario.ForgetTxHash(CInv(MSG_TX, txhash));
    scenario.ForgetTxHash(CInv(MSG_QUORUM_FINAL_COMMITMENT, txhash));
