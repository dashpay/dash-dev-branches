...
/** An announcement. This is the data we track for each inv that is announced to us by each peer. */
struct Announcement {
    /** The inv that was announced. */
    const CInv m_inv;
    ...
    uint8_t m_state : 3 {static_cast<uint8_t>(State::CANDIDATE_DELAYED)};
    ...
    Announcement(const CInv& inv, NodeId peer, bool preferred, std::chrono::microseconds reqtime,
        ...
        m_inv(inv), m_time(reqtime), m_peer(peer), m_sequence(sequence), m_preferred(preferred) {}
};
...
public:
    ...
    {
        return operator()(ann.m_inv, ann.m_peer, ann.m_preferred);
    }
};
...
struct ByPeer {};
using ByPeerView = std::tuple<NodeId, bool, const CInv&>;
struct ByPeerViewExtractor
{
    using result_type = ByPeerView;
    result_type operator()(const Announcement& ann) const
    {
        return ByPeerView{ann.m_peer, ann.GetState() == State::CANDIDATE_BEST, ann.m_inv};
    }
};
...
//   deleted.
struct ByTxHash {};
using ByTxHashView = std::tuple<const CInv&, State, Priority>;
...
public:
    ...
    result_type operator()(const Announcement& ann) const
    {
        ...
        return ByTxHashView{ann.m_inv, ann.GetState(), prio};
    }
};
...
std::map<CInv, TxHashInfo> ComputeTxHashInfo(const Index& index, const PriorityComputer& computer)
{
    std::map<CInv, TxHashInfo> ret;
    for (const Announcement& ann : index) {
        TxHashInfo& info = ret[ann.m_inv];
...
private:
    ...
    void PromoteCandidateReady(Iter<ByTxHash> it)
    {
        ...
        if (it_next == m_index.get<ByTxHash>().end() || it_next->m_inv != it->m_inv ||
    ...
    void ChangeAndReselect(Iter<ByTxHash> it, State new_state)
    {
        ...
        if (it->IsSelected() && it != m_index.get<ByTxHash>().begin()) {
            ...
            if (it_prev->m_inv == it->m_inv && it_prev->GetState() == State::CANDIDATE_READY) {
    ...
    bool IsOnlyNonCompleted(Iter<ByTxHash> it)
    {
        ...
        if (it != m_index.get<ByTxHash>().begin() && std::prev(it)->m_inv == it->m_inv) return false;

        // This announcement has a successor that belongs to the same txhash, and is not COMPLETED.
        if (std::next(it) != m_index.get<ByTxHash>().end() && std::next(it)->m_inv == it->m_inv &&
    ...
    bool MakeCompleted(Iter<ByTxHash> it)
    {
        ...
        if (IsOnlyNonCompleted(it)) {
            ...
            CInv txhash = it->m_inv;
            do {
                it = Erase<ByTxHash>(it);
            } while (it != m_index.get<ByTxHash>().end() && it->m_inv == txhash);
            return false;
        }
    ...
    void SetTimePoint(std::chrono::microseconds now, std::vector<std::pair<NodeId, CInv>>* expired)
    {
        ...
        while (!m_index.empty()) {
            ...
            if (it->GetState() == State::CANDIDATE_DELAYED && it->m_time <= now) {
                ...
                if (expired) expired->emplace_back(it->m_peer, it->m_inv);
...
public:
    ...
    void DisconnectedPeer(NodeId peer)
    {
        ...
        auto it = index.lower_bound(ByPeerView{peer, false, CInv{}});
        ...
        std::vector<std::pair<NodeId, CInv>>* expired)
    {
        ...
        auto it_peer = m_index.get<ByPeer>().lower_bound(ByPeerView{peer, true, CInv{}});
    ...
    void RequestedTx(NodeId peer, const CInv& txhash, std::chrono::microseconds expiry)
    {
        ...
        if (it == m_index.get<ByPeer>().end()) {
            ...
            if (it_old != m_index.get<ByTxHash>().end() && it_old->m_inv == txhash) {
    ...
    bool ReceivedResponse(NodeId peer, const CInv& txhash)
    {
        ...
        if (it == m_index.get<ByPeer>().end() || it->GetState() == State::COMPLETED) return false;
        MakeCompleted(m_index.project<ByTxHash>(it));
        return true;
    }
...
void TxRequestTracker::ForgetTxHash(const CInv& txhash) { m_impl->ForgetTxHash(txhash); }
...
void TxRequestTracker::ReceivedInv(NodeId peer, const CInv& gtxid, bool preferred,
    std::chrono::microseconds reqtime)
{
    m_impl->ReceivedInv(peer, gtxid, preferred, reqtime);
}

void TxRequestTracker::RequestedTx(NodeId peer, const CInv& txhash, std::chrono::microseconds expiry)
{
...
uint64_t TxRequestTracker::ComputePriority(const CInv& txhash, NodeId peer, bool preferred) const
{
