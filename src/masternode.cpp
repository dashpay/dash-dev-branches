...
CMasternode::CMasternode(const uint256 &proTxHash, const CProviderTXRegisterMN& proTx) :
    masternode_info_t{ MASTERNODE_ENABLED, proTx.nProtocolVersion, GetAdjustedTime(),
                       COutPoint(proTxHash, proTx.nCollateralIndex), proTx.addr, CKeyID(), proTx.keyIDMasternode},
    fAllowMixingTx(true)
{
    CTxDestination dest;
...
bool CMasternode::UpdateFromNewBroadcast(CMasternodeBroadcast& mnb, CConnman& connman)
{
    ...
    pubKeyIDMasternode = mnb.pubKeyMasternode.GetID();
    sigTime = mnb.sigTime;
    vchSig = mnb.vchSig;
    ...
    if(fMasternodeMode && pubKeyIDMasternode == activeMasternode.pubKeyIDMasternode) {
...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, const CKeyID& pubkeyID)
{
    int nHeight;
    return CheckCollateral(outpoint, pubkeyID, nHeight);
}

CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, const CKeyID& pubkeyID, int& nHeightRet)
{
    ...
    if(pubkeyID.IsNull() || coin.out.scriptPubKey != GetScriptForDestination(pubkeyID)) {
...
void CMasternode::Check(bool fForce)
{
    ...
    bool fOurMasternode = fMasternodeMode && activeMasternode.pubKeyIDMasternode == pubKeyIDMasternode;
...
void CMasternode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack)
{
    ...
    if (deterministicMNManager->IsDeterministicMNsSporkActive(pindex->nHeight)) {
        ...
        if (!dmn || dmn->state->lastPaidHeight == -1) {
            ...
            LOCK(cs_main);
            nBlockLastPaid = (int)dmn->state->lastPaidHeight;
    ...
    CScript mnpayee = GetScriptForDestination(pubKeyIDCollateralAddress);
...
bool CMasternodeBroadcast::Create(const COutPoint& outpoint, const CService& service, const CKey& keyCollateralAddressNew, const CPubKey& pubKeyCollateralAddressNew, const CKey& keyMasternodeNew, const CPubKey& pubKeyMasternodeNew, std::string &strErrorRet, CMasternodeBroadcast &mnbRet)
{
    ...
    LogPrint("masternode", "CMasternodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyIDMasternode = %s\n",
...
bool CMasternodeBroadcast::SimpleCheck(int& nDos)
{
    ...
    pubkeyScript = GetScriptForDestination(pubKeyIDCollateralAddress);

    if(pubkeyScript.size() != 25) {
        LogPrintf("CMasternodeBroadcast::SimpleCheck -- pubKeyIDCollateralAddress has the wrong size\n");
        nDos = 100;
        return false;
    }

    CScript pubkeyScript2;
    pubkeyScript2 = GetScriptForDestination(pubKeyIDMasternode);

    if(pubkeyScript2.size() != 25) {
        LogPrintf("CMasternodeBroadcast::SimpleCheck -- pubKeyIDMasternode has the wrong size\n");
        nDos = 100;
        return false;
    }
...
bool CMasternodeBroadcast::Update(CMasternode* pmn, int& nDos, CConnman& connman)
{
    ...
    if(pmn->pubKeyIDCollateralAddress != pubKeyIDCollateralAddress) {
        LogPrintf("CMasternodeBroadcast::Update -- Got mismatched pubKeyIDCollateralAddress and outpoint\n");
        nDos = 33;
        return false;
    }
    ...
    if(!pmn->IsBroadcastedWithin(MASTERNODE_MIN_MNB_SECONDS) || (fMasternodeMode && pubKeyIDMasternode == activeMasternode.pubKeyIDMasternode)) {
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if(fMasternodeMode && outpoint == activeMasternode.outpoint && pubKeyIDMasternode == activeMasternode.pubKeyIDMasternode) {
        return false;
    }
    ...
    int nHeight;
    CollateralStatus err = CheckCollateral(outpoint, pubKeyIDCollateralAddress, nHeight);
    ...
    if(err == COLLATERAL_INVALID_PUBKEY) {
        LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should match pubKeyIDCollateralAddress, masternode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
...
bool CMasternodeBroadcast::Sign(const CKey& keyCollateralAddress)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        if (!CHashSigner::VerifyHash(hash, pubKeyIDCollateralAddress, vchSig, strError)) {
        ...
        std::string strMessage = addr.ToString(false) + boost::lexical_cast<std::string>(sigTime) +
                        pubKeyIDCollateralAddress.ToString() + pubKeyIDMasternode.ToString() +
        ...
        if(!CMessageSigner::VerifyMessage(pubKeyIDCollateralAddress, vchSig, strMessage, strError)) {
            LogPrintf("CMasternodeBroadcast::Sign -- VerifyMessage() failed, error: %s\n", strError);
            return false;
        }
    }
...
bool CMasternodePing::Sign(const CKey& keyMasternode, const CKeyID& pubKeyIDMasternode)
{
    std::string strError;
...
bool CMasternodePing::CheckSignature(CKeyID& pubKeyIDMasternode, int &nDos) const
{
    std::string strError = "";
    nDos = 0;
...
bool CMasternodePing::CheckAndUpdate(CMasternode* pmn, bool fFromNewBroadcast, int& nDos, CConnman& connman)
{
    ...
    if (!CheckSignature(pmn->pubKeyIDMasternode, nDos)) return false;
