...
bool IsBasicBLSSchemeActive(const CBlockIndex* pindex)
{
    assert(pindex);

    LOCK(cs_llmq_vbc);
    return VersionBitsState(pindex, Params().GetConsensus(), Consensus::DEPLOYMENT_BASIC_BLS_SCHEME, llmq_versionbitscache) == ThresholdState::ACTIVE;
}
