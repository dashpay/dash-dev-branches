...
std::vector<CFinalCommitment> CDKGSession::FinalizeCommitments()
{
    ...
    for (const auto& p : commitmentsMap) {
        ...
        if (utils::IsQuorumRotationEnabled(fqc.llmqType, m_quorum_base_block_index)) {
            fqc.nVersion = utils::IsBasicBLSSchemeActive(m_quorum_base_block_index) ? CFinalCommitment::BASIC_BLS_INDEXED_QUORUM_VERSION : CFinalCommitment::LEGACY_BLS_INDEXED_QUORUM_VERSION;
            fqc.quorumIndex = quorumIndex;
        } else {
            fqc.nVersion = utils::IsBasicBLSSchemeActive(m_quorum_base_block_index) ? CFinalCommitment::BASIC_BLS_NON_INDEXED_QUORUM_VERSION : CFinalCommitment::LEGACY_BLS_NON_INDEXED_QUORUM_VERSION;
            fqc.quorumIndex = 0;
        }
