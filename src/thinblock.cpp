...
bool CThinBlock::HandleMessage(CDataStream &vRecv, CNode *pfrom)
{
    ...
    // Is there a previous block or header to connect with?
    {
        ...
        if (!ContextualCheckBlockHeader(thinBlock.header, state, Params().GetConsensus(), pprev, thinBlock.header.nTime))
        {
...
bool CThinBlock::process(CNode *pfrom, int nSizeThinBlock)
{
    ...
    if (pfrom->thinBlockWaitingForTxns == 0)
    {
        ...
        LogPrint("Thin", "Reassembled thinblock for %s (%d bytes). Message was %d bytes, compression ratio %3.2f peer=%s\n",
