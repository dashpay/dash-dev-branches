...
public:
    CMempoolAddressDeltaKey(AddressType address_type, uint160 address_bytes, uint256 tx_hash, uint32_t tx_index,
                            bool tx_spent) :
...
public:
    ...
    CAddressIndexKey(AddressType address_type, uint160 address_bytes, int32_t block_height, uint32_t block_tx_pos,
                     uint256 tx_hash, uint32_t tx_index, bool tx_spent) :
        m_address_type{address_type},
        m_address_bytes{address_bytes},
...
public:
    ...
    bool IsNull() const { return (m_amount == -1); }

public:
    SERIALIZE_METHODS(CAddressUnspentValue, obj) { READWRITE(obj.m_amount, obj.m_tx_script, obj.m_block_height); }
};

#endif // BITCOIN_INDEX_ADDRESSINDEX_TYPES_H
