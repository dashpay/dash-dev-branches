...
// static std::shared_ptr<const CGrapheneBlock> most_recent_graphene_block;
...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) 
{
...

    {
        ...
        // most_recent_graphene_block = pgrapheneblock;
    }
...
{
    // TODO: Nakul cs_xval, to another lock
    ...
    for (const uint256 &hash : pfrom->grapheneBlockHashes)
    {
        ...
        if (graphenedata.AddGrapheneBlockBytes(nTxSize, pfrom) > maxAllowedSize)
        {
            ...
            if (connman.ClearLargestGrapheneBlockAndDisconnect(pfrom))
            {
...
void HandleGrapheneBlockMessage(CNode *pfrom, const std::string strCommand, CBlockRef pblock, const CInv inv, CConnman& connman)
{
    uint64_t nSizeBlock = pblock->GetBlockSize();
    int64_t startTime = GetTimeMicros();
    CValidationState state;

    // Indicate that the block was fully received. At this point we have either
    // a block or a fully reconstructed graphene                                                                      
    ...
    if (state.IsInvalid(nDoS))
    {
        ...
        // the current fork is bad due to this block so reset the best header to the
        // best fully-validated block
        // so we can download another fork of headers (and blocks).              
        // ActivateBestChain(state, chainparams);
        //TODO: Nakul, FindMostWorkChain
	...
	LargestBlockSeen(nSizeBlock); // update largest block seen               
        ...
        if ((strCommand != NetMsgType::BLOCK) && fGrapheneBlockEnabled)
        {                                                                        
            LogPrint("Graphene", "Processed Block %s reconstructed from (%s) in %.2f seconds, peer=%d\n",
        ...
        {                                                                        
            LogPrint("Graphene", "Processed Regular Block %s in %.2f seconds, peer=%d\n", inv.hash.ToString(),
                (double)(GetTimeMicros() - startTime) / 1000000.0, pfrom->id);
        }         
    ...
    int nTotalGrapheneBlocksInFlight = 0;
    {
        ...
        nTotalGrapheneBlocksInFlight = connman.TotalGrapheneBlocksInFlight();
    ...
    //TODO: Nakul, ClearOrphanCache
    //PV->ClearOrphanCache(pblock)
    EraseOrphanTx(pblock->GetHash());
...
bool ProcessGrapheneBlock(CNode *pfrom, int nSizeGrapheneBlock, std::string strCommand, CConnman& connman, CGrapheneBlock &grapheneBlock) 
{
    ...
    // after a graphene request, because we would never explicitly request two graphene blocks from the same peer.
    // TODO: Nakul PV
    ...
    {
        LOCK(cs_main);
        fXVal = (grapheneBlock.header.hashPrevBlock == chainActive.Tip()->GetBlockHash()) ? true : false;
    }
    ...
    uint256 nullhash;
    pfrom->grapheneBlock.nVersion = grapheneBlock.header.nVersion;
    pfrom->grapheneBlock.nBits = grapheneBlock.header.nBits;
    pfrom->grapheneBlock.nNonce = grapheneBlock.header.nNonce;
    pfrom->grapheneBlock.nTime = grapheneBlock.header.nTime;
    pfrom->grapheneBlock.hashMerkleRoot = grapheneBlock.header.hashMerkleRoot;
    pfrom->grapheneBlock.hashPrevBlock = grapheneBlock.header.hashPrevBlock;
    pfrom->grapheneBlockHashes.clear();
    pfrom->grapheneBlockHashes.resize(grapheneBlock.nBlockTxs, nullhash);
    ...
    //TODO: Nakul, grapheneBlock.vTxHashes
    grapheneBlock.vTxHashes.reserve(grapheneBlock.nBlockTxs);
    ...
    bool fMerkleRootCorrect = true;
    {
        ...
        for (auto &kv : mapOrphanTransactions)
        {
            ...
            if (!ir.second)
                collision = true; // insert return false if no insertion
	...
	// Add full transactions included in the block
        ...
        if (!collision)
        {
            try
            {
                std::vector<uint64_t> blockCheapHashes = grapheneBlock.pGrapheneSet->Reconcile(mapPartialTxHash);
            ...
            if (setHashesToRequest.empty() && !fRequestFailover)
            {
                ...
                if (grapheneBlock.header.hashMerkleRoot != merkleroot || mutated)
                    fMerkleRootCorrect = false;
                else
                {
                    if (!ReconstructBlock(pfrom, fXVal, missingCount, unnecessaryCount, connman))
                        return false;
                }
            }
        }
    ...
    if (fRequestFailover)
    {
        RequestFailoverBlock(pfrom, grapheneBlock.header.GetHash(), connman);
        return true;
    }
    ...
    if (setHashesToRequest.size() > 0)
    {
        ...
        CRequestGrapheneBlockTx grapheneBlockTx(grapheneBlock.header.GetHash(), setHashesToRequest);
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    std::cout<<"strCommand is : " << strCommand << std::endl;
    ...
    if (strCommand == NetMsgType::VERACK)
    {
       ...
       if (pfrom->nVersion >= GRAPHENE_BLOCK_VERSION){
           ...
           // We send this to non-NODE NETWORK peers as well, because
           // they may wish to request compact blocks from us
           // bool fAnnounceUsingGRAPHENEBLOCK = false;
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        bool fNewBlock = false;
        bool processed = ProcessNewBlock(chainparams, pblock, forceProcessing, &fNewBlock);
        if (fGrapheneBlockEnabled && processed)
        {
            int nTotalGrapheneBlocksInFlight = 0;
            {
                LOCK(pfrom->cs_mapgrapheneblocksinflight);                                   
                if (pfrom->mapGrapheneBlocksInFlight.count(hash))                        
                {
                    pfrom->mapGrapheneBlocksInFlight[hash].fReceived = true;   
                    graphenedata.ClearGrapheneBlockData(pfrom, hash);
    ...
    else if (strCommand == NetMsgType::GETGRAPHENE && !fImporting && !fReindex && fGrapheneBlockEnabled)
    {
        ...
        if (chainparams.NetworkIDString() == "main") // other networks have variable mining rates
        {
            if (pfrom->nGetGrapheneCount >= 20)
            {
                ...
                return error("sending too many GETGRAPHENE messages");
            }
        }
        ...
        if (!(inv.type == MSG_GRAPHENE_BLOCK) || inv.hash.IsNull())
        {
            Misbehaving(pfrom->GetId(), 100);
            return error("invalid GETGRAPHENE message type=%u hash=%s", inv.type, inv.hash.ToString());
        }
        ...
        CBlock block;
        {
            ...
            if (!ReadBlockFromDisk(block, (*mi).second, consensusParams))
            {
                ...
                return error("Peer %d requested block %s that cannot be read",  pfrom->id, inv.hash.ToString());
            }
            else
                SendGrapheneBlock(MakeBlockRef(block), pfrom, inv, connman);
        }
             ...
             fGrapheneBlockEnabled)
    {
        if (!pfrom->GrapheneCapable())
        {
            ...
            return error("%s message received from a non GRAPHENE node, peer=%d", strCommand, pfrom->id);
        }
...

        {
            ...
            // Is there a previous block or header to connect with?
            {
                ...
                if (mi == mapBlockIndex.end()) {
                    return error("Graphene block from peer %s will not connect, unknown previous block %s", pfrom->id,
                                 prevHash.ToString());
                }
            }
            ...
            CValidationState state;
            const CBlockIndex *pindex = NULL;
            if (!ProcessNewBlockHeaders({grapheneBlock.header}, state, Params(), &pindex)) {
                int nDoS;
                if (state.IsInvalid(nDoS)) {
                    if (nDoS > 0)
                        Misbehaving(pfrom->GetId(), nDoS);
            ...
            bool result = ProcessGrapheneBlock(pfrom, nSizeGrapheneBlock, strCommand, connman, grapheneBlock );
             ...
             fGrapheneBlockEnabled)
    {
        ...
        {
            Misbehaving(pfrom->GetId(), 100);
            return error("get_grblocktx message received from a non GRAPHENE node, peer=%d", pfrom->id);
        }
        ...
        {
            Misbehaving(pfrom->GetId(), 100);
            return error("Incorrectly constructed get_grblocktx received.  Banning peer=%d", pfrom->id);
        }
        ...
        // If they make more than 20 requests in 2.5  minutes then disconnect them
        {
            ...
            uint64_t nNow = GetTime();
            pfrom->nGetGrapheneBlockTxCount *= std::pow(1.0 - 1.0 / 150.0, (double)(nNow - pfrom->nGetGrapheneBlockTxLastTime));
            pfrom->nGetGrapheneBlockTxLastTime = nNow;
            pfrom->nGetGrapheneBlockTxCount += 1;
            LogPrint("GRAPHENE", "nGetGrapheneTxCount is %f\n", pfrom->nGetGrapheneBlockTxCount);
        ...
        fGrapheneBlockEnabled)
    {
        ...
        // Message consistency checking
        CInv inv(MSG_GRAPHENE_BLOCK, grapheneBlockTx.blockhash);
        if (grapheneBlockTx.vMissingTx.empty() || grapheneBlockTx.blockhash.IsNull())
        {
            graphenedata.ClearGrapheneBlockData(pfrom, inv.hash);

            Misbehaving(pfrom->GetId(), 100);
            return error("Incorrectly constructed grblocktx or inconsistent graphene block data received.  Banning peer=%d", pfrom->id);
        }

        LogPrint("GRAPHENE", "Received grblocktx for %s peer=%d\n", inv.hash.ToString(), pfrom->id);

        {
            // Do not process unrequested grblocktx unless from an expedited node.
            LOCK(pfrom->cs_mapgrapheneblocksinflight);

            // TODO: Nakul connmgr functionality, && !connmgr->IsExpeditedUpstream(pfrom)
            if (!pfrom->mapGrapheneBlocksInFlight.count(inv.hash) )
            {
                Misbehaving(pfrom->GetId(), 10);
                return error("Received grblocktx %s from peer %d but was unrequested", inv.hash.ToString(), pfrom->id);
            }
        }
        ...
        if (fAlreadyHave)
        {
            // TODO: Nakul, AlreadyReceived(inv)
            // requester.AlreadyReceived(inv);
            graphenedata.ClearGrapheneBlockData(pfrom, inv.hash);

            LogPrint("GRAPHENE", "Received grblocktx but returning because we already have this block %s on disk, peer=%d\n",
                    inv.hash.ToString(), pfrom->id);
            return true;
        }
        ...
        LogPrint("GRAPHENE", "Got %d Re-requested txs from peer=%d\n", grapheneBlockTx.vMissingTx.size(), pfrom->id);
        ...
        if (pfrom->grapheneBlock.hashMerkleRoot != merkleroot || mutated)
        {
            ...
            Misbehaving(pfrom->GetId(), 100);
            return error("Merkle root for %s does not match computed merkle root, peer=%d", inv.hash.ToString(),
                    pfrom->id);
        }
        LogPrint("GRAPHENE", "Merkle Root check passed for %s peer=%s\n", inv.hash.ToString(), pfrom->GetLogName());

        // TODO: Nakul fXVal, else checkhashPrevBlock
        // Xpress Validation - only perform xval if the chaintip matches the last blockhash in the graphene block
        bool fXVal;
        {
            LOCK(cs_main);
        ...
        else
        {
            ...
            // TODO: Nakul, test for correct serialize size of block
            int blockSize = ::GetSerializeSize(pfrom->grapheneBlock, SER_NETWORK, PROTOCOL_VERSION);
