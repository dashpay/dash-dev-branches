        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    PlatformDestination platform_dest;
    if (!IsValidDestination(dest)) {
        platform_dest = DecodePlatformDestination(address);
        ExtractDestination(GetScriptForPlatformDestination(platform_dest), dest);
    }
    const bool is_platform = IsValidPlatformDestination(platform_dest);
    ...
    ret.pushKV("address", is_platform ? EncodePlatformDestination(platform_dest) : EncodeDestination(dest));
    if (is_platform) ret.pushKV("isplatform", true);
